...
unit intf.ZUGFeRDLineStatusCodes;
...
procedure Map (EnumValue: TZUGFeRDLineStatusCodes; StringValue: string); inline;
begin
  TEnumExtensions<TZUGFeRDLineStatusCodes>.RegisterMapping(EnumValue, StringValue)
end;
...
begin
  ...
  Map(Changed,                                         '3');
  Map(NoAction,                                        '4');
  Map(AcceptedWithoutAmendment,                        '5');
  Map(AcceptedWithAmendment,                           '6');
  Map(NotAccepted,                                     '7');
  Map(ScheduleOnly,                                    '8');
  Map(Amendments,                                      '9');
  Map(NotFound,                                        '10');
  Map(NotAmended,                                      '11');
  Map(LineItemNumbersChanged,                          '12');
  Map(BuyerHasDeductedAmount,                          '13');
  Map(BuyerClaimsAgainstInvoice,                       '14');
  Map(ChargeBackBySeller,                              '15');
  Map(SellerWillIssueCreditNote,                       '16');
  Map(TermsChangedForNewTerms,                         '17');
  Map(AbideOutcomeOfNegotiations,                      '18');
  Map(SellerRejectsDispute,                            '19');
  Map(Settlement,                                      '20');
  Map(NoDelivery,                                      '21');
  Map(CallOffDelivery,                                 '22');
  Map(ProposedAmendment,                               '23');
  Map(AcceptedWithAmendmentNoConfirmationRequired,     '24');
  Map(EquipmentProvisionallyRepaired,                  '25');
  Map(Included,                                        '26');
  Map(Inquiry,                                         '34');
  Map(Checked,                                         '35');
  Map(NotChecked,                                      '36');
  ...
  Map(Agreed,                                          '40');
  Map(Proposed,                                        '41');
  Map(AlreadyDelivered,                                '42');
  Map(AdditionalSubordinateStructuresWillFollow,       '43');
  Map(AdditionalSubordinateStructuresWillNotFollow,    '44');
  Map(ResultOpposed,                                   '45');
  Map(AuctionHeld,                                     '46');
  Map(LegalActionPursued,                              '47');
  Map(MeetingHeld,                                     '48');
  Map(ResultSetAside,                                  '49');
  Map(ResultDisputed,                                  '50');
  Map(Countersued,                                     '51');
  Map(Pending,                                         '52');
  Map(CourtActionDismissed,                            '53');
  Map(ReferredItemAccepted,                            '54');
  Map(ReferredItemRejected,                            '55');
  Map(DebitAdviceStatementLine,                        '56');
  Map(CreditAdviceStatementLine,                       '57');
  Map(GroupedCreditAdvices,                            '58');
  Map(GroupedDebitAdvices,                             '59');
  Map(Registered,                                      '60');
  Map(PaymentDenied,                                   '61');
  Map(ApprovedAsAmended,                               '62');
  Map(ApprovedAsSubmitted,                             '63');
  Map(CancelledNoActivity,                             '64');
  Map(UnderInvestigation,                              '65');
  Map(InitialClaimReceived,                            '66');
  Map(NotInProcess,                                    '67');
  Map(RejectedDuplicate,                               '68');
  Map(RejectedResubmitWithCorrections,                 '69');
  Map(PendingIncomplete,                               '70');
  Map(UnderFieldOfficeInvestigation,                   '71');
  Map(PendingAwaitingAdditionalMaterial,               '72');
  Map(PendingAwaitingReview,                           '73');
  Map(Reopened,                                        '74');
  Map(ProcessedByPrimaryForwardedToAdditionalPayers,   '75');
  Map(ProcessedBySecondaryForwardedToAdditionalPayers, '76');
  Map(ProcessedByTertiaryForwardedToAdditionalPayers,  '77');
  Map(PreviousPaymentDecisionReversed,                 '78');
  Map(NotOurClaimForwardedToAnotherPayers,             '79');
  Map(TransferredToCorrectInsuranceCarrier,            '80');
  Map(NotPaidPredeterminationPricingOnly,              '81');
  Map(DocumentationClaim,                              '82');
  Map(Reviewed,                                        '83');
  Map(Repriced,                                        '84');
  Map(Audited,                                         '85');
  Map(ConditionallyPaid,                               '86');
  Map(OnAppeal,                                        '87');
  Map(Closed,                                          '88');
  Map(Reaudited,                                       '89');
  Map(Reissued,                                        '90');
  Map(ClosedAfterReopening,                            '91');
  Map(Redetermined,                                    '92');
  Map(ProcessedAsPrimary,                              '93');
  Map(ProcessedAsSecondary,                            '94');
  Map(ProcessedAsTertiary,                             '95');
  Map(CorrectionOfError,                               '96');
  Map(SingleCreditItemOfAGroup,                        '97');
  Map(SingleDebitItemOfAGroup,                         '98');
  Map(InterimResponse,                                 '99');
  Map(FinalResponse,                                   '100');
  Map(DebitAdviceRequested,                            '101');
  Map(TransactionNotImpacted,                          '102');
  Map(PatientToBeNotified,                             '103');
  Map(HealthcareProviderToBeNotified,                  '104');
  Map(UsualGeneralPractitionerToBeNotified,            '105');
  Map(AdviceWithoutDetails,                            '106');
  Map(AdviceWithDetails,                               '107');
  Map(AmendmentRequested,                              '108');
  Map(ForInformation,                                  '109');
  Map(Withdraw,                                        '110');
  Map(DeliveryDateChange,                              '111');
  Map(QuantityChange,                                  '112');
  Map(ResaleAndClaim,                                  '113');
  Map(Resale,                                          '114');
  Map(PriorAddition,                                   '115');
  Map(Expired,                                         '116');
  Map(Hold,                                            '117');
  Map(Open,                                            '118');
{.MapEnd}
